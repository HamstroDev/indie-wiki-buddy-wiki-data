...
# This script bundles all data needed by IWB into a single `data.json` file.
# It also generates a `meta.json` file describing `data.json`, which the
# extension queries to check whether it should update its cached data.
...
format_version = "4.0-dev"

with open('breezewiki-instances/instances.json', 'r') as instances_json:
    breezewiki_instances = json.load(instances_json)
...
data = {
    ...
    'breezewiki_instances': breezewiki_instances,
    'hosts': {},
    'wikis': wikis,
}
